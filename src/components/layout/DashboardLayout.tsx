  ...
  const navItems =
    language === "en"
      ? [
          ...
          { icon: Sparkles, label: "New Analysis", href: "/dashboard/new-analysis" },
    ...
    (href: string) =>
      ...
      (href === "/dashboard/history" &&
        (location.pathname === "/dashboard/tenders" ||
          ...
          location.pathname.startsWith("/dashboard/informe/")));
    ...
    <div className="min-h-screen bg-muted/30 flex">
      ...
      <main className="flex-1 min-w-0">
        <header className="bg-card border-b border-border px-6 py-4 flex items-center gap-4 lg:hidden">
          <button className="text-foreground" onClick={() => setSidebarOpen(true)}>
            <Menu size={24} />
          </button>
        </header>
        {children}
